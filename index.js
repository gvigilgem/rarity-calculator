...
<script>
  ...
  async function logEntry(data) {
    try { await fetch(`${SUPABASE_URL}/rest/v1/honeypot_logs`, {
      method:"POST", headers:{
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    term.innerHTML = \`
      <div style="color:#8b0000;font-weight:bold">UNCLASSIFIED // FOUO</div>
      <div>IWC // INTELLIGENCE WARFARE CELL</div>
      <div>POLICY DIRECTIVE 0219D</div>
      <div style="color:#ff0;">*** WARNING: 3 failed attempts = SYSTEM LOCKOUT (18 USC § 1030) ***</div>
      <br>Last login: Tue Dec 2 23:59:01 2025 from Cloudflare<br>
      login: \`;
    input.focus();
  ...
  const timer = setInterval(() => {
    timeLeft--;
    document.getElementById('timer').innerText = \`SYSTEM LOCKOUT IN \${timeLeft}s\`;
