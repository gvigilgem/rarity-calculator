export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/portal") {
      return new Response(portalHtml.replace("{{SUPABASE_URL}}", SUPABASE_URL), {
...
<button onclick="fetch('{{SUPABASE_URL}}/rest/v1/documents',{
  headers:{
    'apikey':localStorage.getItem('sb-access'),
    'Authorization':'Bearer '+localStorage.getItem('sb-access')
  }})
  .then(r=>r.json()).then(d=>alert('Documents: '+JSON.stringify(d)))" 
